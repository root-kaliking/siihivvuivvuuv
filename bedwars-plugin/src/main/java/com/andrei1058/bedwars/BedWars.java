...
public class BedWars extends JavaPlugin {
    ...
    /**
     * Detects the NMS version of the running server.
     * <p>
     * On Spigot/older Paper the server class ships inside a versioned craftbukkit package
     * (e.g. {@code org.bukkit.craftbukkit.v1_20_R3.CraftServer}) and the version segment
     * is taken directly from the class name. On Paper 1.21+ the package is no longer
     * versioned (e.g. {@code org.bukkit.craftbukkit.CraftServer}), so we fall back to the
     * Bukkit version string and map known versions to their NMS modules.
     */
    private static String detectServerVersion() {
        String pkg = Bukkit.getServer().getClass().getName().split("\\.")[3];
        if (pkg.matches("v\\d+_\\d+_R\\d+")) {
            return pkg;
        }
        // Paper 1.21+: no versioned craftbukkit package anymore
